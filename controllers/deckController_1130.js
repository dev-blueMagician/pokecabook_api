...
exports.getDecks = async (req, res) => {
  ...
  try {
        ...
        for(let i = 0; i < conditions.length; i++){
          ...
          if (conds.length > 0) {
              conds.forEach((item,index) => {
                  ...
                  whereConditions += `(c.name_var = '${item.cardName}' AND c.count_int ${operator} '${item.cardNumber}')`;
              })
          }
    ...
    if(filter.cardName){
      whereConditions += ` AND c.name_var LIKE '%${filter.cardName}%'`;
    }
    if(filter.cardNumMin){
      whereConditions += ` AND c.count_int >= '${filter.cardNumMin}'`;
    }
    if(filter.cardNumMax){
      whereConditions += ` AND c.count_int <= '${filter.cardNumMax}'`;
    }
    ...
    query = `
      ...
      FilteredValidDecks AS (
          SELECT DISTINCT fd.deck_ID_var
          FROM FilteredDecks fd
          LEFT JOIN cards c ON fd.deck_ID_var = c.deck_ID_var
          WHERE ${whereConditions}
      )
      SELECT d2.*, e.event_prefecture
      FROM FilteredValidDecks fvd
      INNER JOIN decks d2 ON fvd.deck_ID_var = d2.deck_ID_var
