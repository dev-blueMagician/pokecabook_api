...
exports.getCards = async (req, res) => {
  ...
  try {
    ...
    const { startDate: filterStartDate, endDate: filterEndDate, league, category, ranks, prefectures } = req.body.filter;
    ...
    const events_count_query = `
            ...
            if (conds.length > 0) {
              ...
              const [specific_decks_count] = await db.query(specific_decks_count_query, [startDate, endDate, league]);
              console.log(specific_decks_count)
              filtered_specific_decks_count = specific_decks_count[0]?.specific_count || 0;
            } else {
